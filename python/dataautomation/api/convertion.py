...
def get_files(in_filename):
    data = pd.read_excel(in_filename,header=None)
    return data

#extract filenames
def filenameextract(in_filename):
    filename=os.path.basename(in_filename).split('/')[-1].split('.')[0]
    return filename
...
def mainstart(inpath,outpath="output")->dict:

    filename='output'
    print(inpath.name)

    # creating output folder if doesnt exist
    Path("./output/").mkdir(parents=True, exist_ok=True) 

    try:
        df = pd.read_excel(inpath)
        df=dfprepare(df,filename)
        df=formatting(df)
        df.to_excel(outpath+"/"+filename +"output.xlsx",index=False,header=False)
