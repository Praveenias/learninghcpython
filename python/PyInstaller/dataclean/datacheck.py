...
class Datacheck:
    ...
    def __init__(self) -> None:
        ...
        self.root_path:Path = Path(__file__).resolve().parent
        self.input_data = {}
    ...
    def choose_template(self)->dict:
        ...
        try:
            with open(Path.joinpath(self.root_path,self.template_json_path),encoding='utf-8') as fd:
    ...
    def get_fmd_issue(self,data_d:dict,template_data:dict,check_clm:str)->str:
    ...
    def data_type_check(self,dataType:str,data,clmName:str)->str:
        ...
        if dataType == 'date':
            try:
                ...
                # else:
                    datetime.strptime(data,"%Y-%b-%d") #dd-mmm-yyyy
            except Exception as e:
    ...
    def check_data(self,row_data:dict,template_data:dict,is_fmd:bool) -> dict:
    ...
    def write_issue(self,headername:list , ws:openpyxl.worksheet,
        ...
        is_fmd = True if self.input_data['file_type'] == 'fmd' else False
        ...
        try:          
            ...
            for data in ws.iter_rows(min_row=headerRow+1,max_col=len(headername),
                ...
                if is_fmd:
                    ...
                    if True in clm_data_value:
                        issue = self.get_fmd_issue(data_d,template_data,"req-field-1")         
                    else :
                        clm_data_value = [True for clm_name in template_data['req-field-2'] if data_d[clm_name]!=None ]
                    if True in clm_data_value:
                        issue = self.get_fmd_issue(data_d,template_data,"req-field-2")
                    else:
                        clm_data_value = [True for clm_name in template_data['req-field-3'] if data_d[clm_name]!=None ]
                        issue = self.get_fmd_issue(data_d,template_data,"req-field-3")
                #print(data_d)
                issue = self.check_data(data_d,template_data,is_fmd)
            ...
            #logger.warning(f'Excel Processing error: {err}')
            result['status'] = 'error'
            result['msg'] = f'Excel Processing error: {err}'
            return result

        return result
    def main(self,input:dict) -> dict:
        """Main Method for parsing the input  
        ...
        logger.info(__file__+ f" inputs: f{input}")
        if result['status'] == 'success':
            self.input_data = input
            template_fields = self.choose_template()
            if (template_fields):
                result = self.check_main(template_fields)
                logger.info(__file__+ f' Output : {result}')
                if result['status'] =='success':
                    result['msg'] = 'Excel is Successfully1 parsed, Saved in Output folder'
            else:
                logger.error(__file__+ f' failure : {result}')
                result['status']='error'
