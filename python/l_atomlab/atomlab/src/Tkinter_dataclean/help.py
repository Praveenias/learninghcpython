...
out_dir = Path("./logs/").mkdir(parents=True, exist_ok=True)
log_file_name = Path.joinpath(Path('./logs/'),'AtomHelperLog.log')
logger.basicConfig(filename = log_file_name,
                    filemode = "a",
                    format = Log_Format,
                    level=logging.INFO)
...
class Helper:
    ...
    def check_db_connection(self,db_details:dict)->bool:
        """check the db connection from db_details 

        Parameters
        ----------
        db_details  : dict , mandatory
            connection parameters

        Returns
        -------
        Bool
            success or failure connection
