...
def gen_outfpath(out_filename:str)->Path:
  '''
  Note : creating file location where the output file will save
  '''
  if not out_filename:
...
def data_type_check(dataType:str,data,clmName:str)->str:
  '''validating single column's additional checks mentioned in rule_template  '''
...
def check_data(row_data:dict,template_data:dict) -> dict:
  '''
  Validating single row .. if issue found
  returning issue 
  '''
...
def write_issue(headername:list , ws:openpyxl.worksheet,
  ...
  ''' extracting single row from excel validating then writing in issue clm
  checking for uniqueness , if dupilicate found writing in duplicate clm
  '''
  result = ''
  ...
  try:          
    ...
    for data in ws.iter_rows(min_row=headerRow+1,max_col=len(headername),
      ...
      issue = check_data(data_d,template_data)
...
def get_mandatory_columns(template_data:dict) -> list:
  '''
    We can use lambda, instead of this function. But this will make
    maintenance simple
  '''
  result = []
...
def check_main(args:argparse.Namespace , template_data:dict) -> str:
  '''validating input excel worksheet and header columns'''
...
def choose_template(template_fname:str, input_datatype:str)->dict:
  '''getting rules for user input's excel file
     from json file'''
  try:
...
def create_parser(allowed_datatype:list,file_type:list) -> argparse.Namespace:
  '''
...
if __name__ == '__main__':
  ...
  ip_args = create_parser(allowed_datatype,allowed_filetype)
  logging.info(f"input arguments{ip_args}")
  
  if not Path(Path.joinpath(BASE_DIR,template_fname[ip_args.filetype])).is_file():
    print(f'{template_fname[ip_args.filetype]}: Template file to check data is not present ')
  else:   
    result = check_input(ip_args)
    if (not result):
      template_fields = choose_template(template_fname[ip_args.filetype], ip_args.datatype)
...
#python datacheck.py --datatype bom --inputfile ../data/datacheck_bom_com.xlsx --outfilename result_bom --filetype compliance
#python datacheck.py --datatype master --inputfile ../data/datacheck_master_com.xlsx --outfilename result_master --filetype compliance
#python datacheck.py --datatype substance --inputfile ../data/datacheck_substance.xlsx --outfilename result_substance --filetype compliance
#python datacheck.py --datatype regulation --inputfile ../data/datacheck_regulation.xlsx --outfilename result_regulation --filetype compliance
#python datacheck.py --datatype manufacturer --inputfile ../data/datacheck_manufacturer.xlsx --outfilename result_manufacturer --filetype compliance
#python datacheck.py --datatype contact --inputfile ../data/datacheck_contact.xlsx --outfilename result_contact --filetype compliance
